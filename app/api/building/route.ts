...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [
      ...
      lat && lng ? fetchData('drex-xx56', `$where=within_circle(the_geom,${lat},${lng},1000)&$limit=50`).catch(()=>[]) : Promise.resolve([]), // Subway entrances nearby
      Promise.resolve([]), // Skip bus stops 
      lat && lng ? fetchData('dzhx-5ksa', `$where=within_circle(the_geom,${lat},${lng},800)&$limit=30`).catch(()=>[]) : Promise.resolve([]), // Citi Bike nearby
